  ...
  //  *p_err = OS_ERR_NONE; 
   ...
   // OS_CRITICAL_ENTER();
    ...
    OS_CRITICAL_EXIT_NO_SCHED();
     ...
     TEMP[counttemp].period = h.TASK[0]->period;
     TEMP[counttemp].data = (h.TASK[0]->period + OSTickCtr);
     TEMP[counttemp].p_tcb = p_tcb;
     heap_pop(&h);
     heap_push(&h,&TEMP[counttemp]);
     counttemp++;
  }    
